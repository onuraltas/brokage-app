...
import com.challenge.util.WebUtils;
...
public class OrderServiceImpl implements OrderService {
    ...
    private OrderEntity createOrder(String assetName, Side orderSide, BigDecimal size, BigDecimal price) {
        ...
        if(assetName.equals(AssetUtil.TRY)) {
            throw new BrokageException(ExceptionMessage.TRY_ASSET_CANNOT_BE_TRADED);
        }

        if(!AssetUtil.isValidAsset(assetName)) {
            throw new BrokageException(ExceptionMessage.UNKNOWN_ASSET, assetName);
        }
        ...
        if(Side.SELL.equals(orderSide)) {
            var existingAssetEntity = assetRepository
                            ...
                            .orElseThrow(() -> new BrokageException(ExceptionMessage.ASSET_NOT_FOUND));

            var existingTryAssetEntity = assetRepository
                    .findByCustomerIdAndAssetName(newOrderEntity.getCustomerId(), AssetUtil.TRY)
                    .orElseThrow(() -> new BrokageException(ExceptionMessage.PARAM_ASSET_NOT_FOUND, AssetUtil.TRY));

            if(existingAssetEntity.getUsableSize().subtract(size).compareTo(BigDecimal.ZERO) < 0) {
                throw new BrokageException(ExceptionMessage.ASSET_NOT_ENOUGH);
            }
            ...
            if(existingTryAssetEntity.getUsableSize().subtract(size.multiply(price)).compareTo(BigDecimal.ZERO) < 0) {
                throw new BrokageException(ExceptionMessage.PARAM_ASSET_NOT_ENOUGH, AssetUtil.TRY);
            }
            ...
            throw new BrokageException(ExceptionMessage.ORDER_SIDE_IS_FALSE);
        }
    ...
    public void cancelOrder(Integer orderId) {
        var existingOrderEntity = orderRepository.findById(orderId).orElseThrow(() -> new BrokageException(ExceptionMessage.RECORD_NOT_FOUND));

        var customerId = webUtils.getLoginnedCustomerId();

        if(!customerId.equals(existingOrderEntity.getCustomerId())) {
            throw new BrokageException(ExceptionMessage.ORDER_DOES_NOT_BELONG_TO_THIS_USER);
        }
        ...
        if(existingOrderEntity.getStatus().equals(Status.PENDING)) {
            ...
            throw new BrokageException(ExceptionMessage.ORDER_CANNOT_BE_CANCELLED);
        }
    }
